...
NOTE: This script (to be precise: the classification, and building up from the
classifications results, and the plotting of confusion matrices)
is relying on two functionalities in pymvpa that so far only
live in a PRs! Execute the script in an appropriate virtualenv!
...
def dotheclassification(ds,
   ...
   # import matplotlib.pyplot as plt
    ...
    if bilateral:
        ...
        targets = 'all_ROIs'
    gnb = mv.GNB(common_variance=True,
                 prior=prior,
                 space=targets)
...
def dotheglm(sensitivities,
    ...
    # TODO in every fold, divide sensitivities by its L2 norm (so that sensitivities add up to 1)
