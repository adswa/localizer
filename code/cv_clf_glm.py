    ...
    if bilateral:
        for j in range(len(hrf_estimates.fa.bilat_ROIs_str)):
            if classifier in sgds:
                ...
                comparison = hrf_estimates.fa.bilat_ROIs
            if (roi_pair[0] in comparison) and (roi_pair[1] in comparison):
                roi_pair_idx = j
    else:
        for j in range(len(hrf_estimates.fa.all_ROIs_str)):
            if classifier in sgds:
                comparison = hrf_estimates.fa.targets
            else:
                comparison = hrf_estimates.fa.all_ROIs
            if (roi_pair[0] in comparison) and (roi_pair[1] in comparison):
                roi_pair_idx = j
    return roi_pair_idx
...
if __name__ == '__main__':
    ...
    parser.add_argument('--normalize', help="Should the sensitivities used for the glm be"
                        "normalized by their L2 norm? True/False", type=str, default=True)
    ...
    ds_type = args.dataset                      # stripped --> no brain, no overlap,
                                                #  full --> no overlap
    glm = args.glm                              # True or False
    bilateral = args.bilateral                  # True or False
    niceplot = args.niceplot                    # False or True
    normalize = args.normalize                  # True or False
    plot_ts = args.plot_time_series             # False or True
    incl_regs = args.include_all_regressors     # False or True
