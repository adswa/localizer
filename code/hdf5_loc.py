...
def classification_rois_only(ds):
...
def classification_with_coords(ds, full = False):
    ...
    def store_sens(data, node, result):
        ...
        sens.fa['time_coords']=data.fa['time_coords']
        sens.fa['chunks']=data.fa['chunks']
        print('Storing sensitivity for data on participants %s' %str(data.sa['participant'].unique))
...
def makeaplot(events, mean_sens_transposed, hrf_estimates, roi_pair):#, cv):
    """
...
# rerun with coordinates without fmri data
#ds_empty = ds.copy('deep')
#testshape = ds_empty.samples.shape
#test_ds.samples = np.zeros(shape=testshape)
#targets = 'all_ROIs'
#clf_no_fmri = mv.GNB(space=targets, prior='ratio', common_variance=True)
#cv_no_fmri = mv.CrossValidation(clf_no_fmri,
#                         mv.NFoldPartitioner(attr='participant'),
#                         errorfx=mv.mean_match_accuracy,
#                         enable_ca=['stats'])
#results_no_fmri = cv_no_fmri(test_ds)
